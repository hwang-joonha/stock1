...
**개요 차트 다이제스트.** 투자 개요 맨 아래에 적정가 경로·괴리 분해·시나리오
팬·TTM 롤링·배수 밴드·장기 사이클 6장이 함의 1~2줄과 함께 자동으로 모인다.
함의의 숫자는 화면을 그릴 때 모델·시장 블록에서 다시 읽는다 — 손으로 적은
숫자가 없다. 해당 데이터 블록이 없는 차트는 조용히 빠진다. 분기 표·분기
스냅숏의 성장률(YoY·QoQ)은 기저 ≤ 0이면 —, 부호가 뒤집히면 적자/흑자 전환으로
표기한다.
