...
| `LONGHIST` | 사업 구조 뷰의 장기 사이클·현금흐름·재무현황 차트 (빌드가 longhist.json에서 주입) | 카드가 조용히 빠진다 |
| `REGIONS` | 사업 구조 뷰의 지역별 매출 카드 (빌드가 regions.json에서 주입) | 카드가 조용히 빠진다 — 지역별 공시가 없는 종목은 파일 자체가 없다 |
...
**수주 공시·수주잔고.** `MEMO.orders = [{d, what, amt(억원), span?, buyer?,
note?}]`는 판매공급계약 공시(DART 확정 — note에 접수번호를 남긴다),
`MEMO.backlog = [{q:'YYYYQn', amt}]`는 분기보고서의 수주잔고 추이. 분기
모니터링에 "확보된 미래 매출" 카드로 뜨고, 합계·잔고를 마지막 실적 연도 매출
대비 %로 읽는다. 계약 상대가 공시 비공개면 추정 출처를 표기한다. G11이 날짜·
분기 형식과 금액 숫자를 검사한다.

**가격 스냅숏 `PRICES`.** `python3 tools/price_fetch.py`가 야후 파이낸스에서
10년 월간 조정 종가(+ KOSPI `^KS11` 지수)를 받아 `prices.json`으로 두면
빌드가 주입한다. 근사 시총 = 조정 종가 × 현재 상장주식수 — 유상증자·소각에
따른 주식수 변화분은 오차로 남고 `_기준`과 카드 부제에 명시한다. 판단
수치가 아니라 맥락 차트다(밸류에이션 뷰의 실적 대 시총·배수 밴드·시장 대비
상대 성과, 분기 모니터링의 주가 YoY 대 실적 YoY). 모델 비투입.

**장기 실적 `LONGHIST`.** `python3 tools/build_longhist.py`가 사업보고서
3~4개의 3개년(당기/전기/전전기) 블록을 이어 붙여 10년+ 매출·영업이익과
현금흐름(영업/투자/재무·CAPEX·FCF = 영업CF − CAPEX)·재무상태(부채총계·
자본총계) 시계열을 `longhist.json`으로 만들고 빌드가 주입한다. 각 블록은
그 보고서 공시 기준 그대로 — 소급 재작성이 있으면 블록 경계에서 기준이
갈릴 수 있고 `_기준`에 남긴다. 모델 실적 구간과 겹치는 연도의 매출·영업이익은
historicals와 대사해 어긋나면 빌더가 죽는다. 옛 보고서에서 현금흐름·재무상태
행을 못 읽으면 그 연도만 null로 남기고 산다. 사업 구조 뷰 맨 뒤의 사이클·
현금흐름·재무현황 카드가 소비한다. 모델 비투입.

**지역별 매출 `REGIONS`.** `python3 tools/build_region.py`가 사업보고서
주석의 '지역에 대한 공시'(XBRL 행형·고전 열형 두 형태)를 이어 붙여
`regions.json`으로 만들고 빌드가 주입한다. Σ지역 = 합계, 합계 = 연결
매출(겹치는 연도) 대사를 통과해야 저장되며, 빈 셀(소계 열)이 끼어 이름
매핑이 안전하지 않은 변형은 받지 않는다 — 그 보고서만 건너뛴다. 지역별
공시가 없는 종목은 파일을 만들지 않는다.
...
historicals 연간을 대사해 어긋나면 죽는다. 매출원가·순이익(전체 기준,
이익률 차트 전용)도 같은 방법으로 뽑는다 — 부문 주석 종목(삼성전기)은
기존 quarterly.json에 병합만 하고, 부문합 = 손익계산서 매출 대사(0.1억)를
통과해야 한다. 매출원가를 XBRL 부호(음수)로 공시하는 회사는 크기로
정규화한다.

**개요 차트 다이제스트.** 투자 개요 맨 아래에 결론을 만드는 차트들이 함의
1~2줄과 함께 자동으로 모인다 — 적정가 경로·괴리 분해·시나리오 팬·TTM 롤링·
분기 이익률·배수 밴드·시장 대비 상대 성과·장기 사이클·지역별 매출·현금흐름·
재무현황(최대 11장). 함의의 숫자는 화면을 그릴 때 모델·시장 블록에서 다시
읽는다 — 손으로 적은 숫자가 없다. 해당 데이터 블록이 없는 차트는 조용히
빠진다. 분기 표·분기 스냅숏의 성장률(YoY·QoQ)은 기저 ≤ 0이면 —, 부호가
뒤집히면 적자/흑자 전환으로 표기한다.

**차트 디자인 규약.** 선 차트의 마커는 속 빈 원(흰 채움 + 색 테두리), 월간처럼
점이 많은 시계열은 마지막 점만. 다중 선의 범례는 하단 중앙. 막대+선 이중 축은
오른쪽에 보조축 눈금 5개. 추정 구간 막대는 옅은 색 + 투명도. 색은 여전히
tokens.js 램프에서만 — 다색 팔레트는 쓰지 않는다(색에 실린 의미 체계 유지).
